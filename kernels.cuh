...
__global__ void relax(int N, float MAX_VAL, int *d_in_V, int *d_in_I, int *d_in_E, float *d_in_W, float *d_out_D, float *d_out_Di); //change myself
__global__ void updateDistance(int N, int *d_in_V, int *d_in_I, int *d_in_E, float *d_in_W, float *d_out_D, float *d_out_Di); //change myself
__global__ void updateIndexOfEdges(int N, int *d_in_V, int *d_in_E, int l, int r);
__global__ void updatePred(int N, int *d_in_V, int *d_in_I, int *d_in_E, float *d_in_W, float *d_out_D, int *d_out_P);
__global__ void initializeArray(const int N, float *p, const float val, bool sourceDifferent, const int source, const int sourceVal);
__global__ void initializeArray1(const int N, int* p, const int val, bool sourceDifferent, const int source, const int sourceVal);
